class Parser:
    ...
    def parseFile(self, filename):
        ...
        if not self._parseLines():
            ...
            Parse._error("File",-1,"Missing undefined labels")
    ...
    def _parseLines(self):
        ...
        for (line, n) in self._lines:
            ...
            if len(l) > 0:
                lines.append(l)
        for label in self._toCheckLabels:
    ...
    def _parseLine(self, line, n):
        ...
        if l[0] == "push":
            if len(l) == 3:
                if l[2].isdigit() and l[2][0]!="-":
                    return "//" + " ".join(l) + "\n" + self._push(l[1], l[2], n)
                else:
                    self._flag=False
                    Parser._error("Parser",n,"Argument must be nonegative integer")
                    return ""
            else:
                self._flag = False
            ...
            if l[0] == "label" and len(l) == 2:
                ...
                self._toCheckLabels.append(l[1])
                return "//" + " ".join(l) + "\n" + self._goto(l[1], n)
