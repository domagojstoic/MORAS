    ...
    def _parseInt(self):
        ...
        while len(self._line):
            ...
            if c in numberChars:
                ret = ret * 10 + int(c)
                if ret>=32768:
                    raise JackError("Syntax error in line "+str(self._lineNum)+":const integer " + c + "cannot be greater than 32767")
            else:
                for x in self._line:
                    if x in numberChars:
                        raise JackError("Syntax error in line "+str(self._lineNum)+"integer cannot contain non-integer characters")
                    if x==" ":
                        break
                break
            self._line = self._line[1:]
